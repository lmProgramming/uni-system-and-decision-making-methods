...
def is_matrix_square(mat: List[List[int]]) -> bool:
    ...
    """
    vertical_length = len(mat)
    
    for line in mat:
        if vertical_length != len(line):
            return False
    return True
...
def matrix_determinant(mat: List[List[int]]) -> int:
    ...
    return matrix_determinant_assume_square(mat)
    

def matrix_determinant_assume_square(mat: List[List[int]]) -> int:
    determinant: int = 0
    ...
    for x, cell in enumerate(line):
        minor = get_minor_matrix(mat, y, x)
        determinant_unsigned = matrix_determinant_assume_square(minor)
