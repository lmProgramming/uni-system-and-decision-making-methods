...
from matrix_math import multiply, inverse, transpose
...
def get_polynomial_form(polynomial_degree: int) -> np.ndarray:
    ...
    """
    x = np.arange(polynomial_degree + 1)
    return np.array(np.vstack(x))

def format_float_with_precision(value: float, precission: int) -> str:
    num_with_specified_precision: str = f"{value:.{precission}f}"
    num_without_traling_zeroes = num_with_specified_precision.rstrip('0')
    
    if num_without_traling_zeroes.endswith(".") and precission > 0:
        num_without_traling_zeroes += "0"
    return num_without_traling_zeroes
...
def print_polynomial(theta: np.ndarray, precission: int = 3) -> str:
    ...
    for i, num in enumerate(theta): 
        string_nums.append(format_float_with_precision(num[0], precission) + "*x^" + str(i))
...
def least_squares_solution(
    ...
    """
    Compute theta mat with coefficients of polynomial fitted by LSS.

    :param X: argument vector, shape = (N, )
    :param Y: target vector, shape = (N, ) 
    :param polynomial_degree: degree of fitted polynomial

    :return: theta mat of polynomial, shape = (1, polynomial_degree + 1)
    """
    polynomial_form_mat = get_polynomial_form(polynomial_degree)
    X_mat = np.column_stack(X**polynomial_form_mat)
    
    X_T = transpose(X_mat)
    
    result = inverse(multiply(X_T, X_mat))
    
    result = multiply(multiply(result, X_T), Y.reshape(-1, 1))
...
def generalised_linear_model(X: np.ndarray, T: np.ndarray) -> np.ndarray:
    ...
    :param X: argument vector, shape = (N, )
    :param T: theta mat of polynomial, shape = (1, polynomial_degree + 1)
...
if __name__ == "__main__":
    ...
    T = least_squares_solution(X, Y, 30)
