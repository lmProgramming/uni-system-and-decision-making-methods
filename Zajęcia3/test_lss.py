...
@pytest.mark.parametrize(
    ...
    [
        (
            np.array([[1.86548519e+00],[0.33807518e-02],[-1.04156655e-05]]), 
            3, 
            "1.865*x^0 + 0.003*x^1 + -0.0*x^2",
        ),
        (
            np.array([
                [4.35169575e+01], [ 1.30404047e-03], [-3.50173017e-08],
                [3.55083643e-13], [-7.97290264e-19], [-3.88362946e-24]
            ]), 
            5, 
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (2, np.array([[4.86548519e+00], [2.33807518e-02], [-5.04156655e-05]])),
        (
            3, 
            np.array([
                [4.79214481e+00], [2.58358036e-02], 
                [-6.95863178e-05], [3.94648205e-08]
            ])
        ),
        (
            4,
            np.array([
                [4.99949649e+00], [1.57674472e-02], [6.45111445e-05],
                [-5.99332140e-07], [9.72509213e-10]
            ])
        ),
        (
            5,
...
def test_least_squares_solution(polynomial_degree, exp_result):
    np.testing.assert_almost_equal(
        lss.least_squares_solution(X, Y, polynomial_degree),
        exp_result,
        decimal=4
    )
